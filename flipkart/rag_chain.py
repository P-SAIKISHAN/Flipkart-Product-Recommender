...
class RAGChainBuilder:
    ...
    def build_chain(self):
        ...
        history_aware_retriever = context_prompt | self.model | RunnableLambda(
            lambda x: retriever.invoke(x if isinstance(x, str) else str(x))
        )
        ...
        rag_chain = (
            RunnablePassthrough.assign(context=lambda x: format_docs(
                retriever.invoke(x.get("input", ""))
            ))
